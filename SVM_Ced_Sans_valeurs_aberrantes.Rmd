...
outlier_mask <- apply(data_subset %>% select(-DIFF), 2, detect_outliers)
outlier_rows <- apply(outlier_mask, 1, any)
data_subset_clean =data_subset[!outlier_rows,]
nrow(data_subset_clean)
nrow(data_subset)
```
...
for (c_val in C_values) {
  ...
  # créer les folds
  folds <- createFolds(data_subset_clean$DIFF, k = nfolds)
  f1_fold <- c()
  
  cm_sum <- matrix(0, nrow = length(levels(data_subset_clean$DIFF)), ncol = length(levels(data_subset_clean$DIFF)))
  rownames(cm_sum) <- levels(data_subset_clean$DIFF)
  colnames(cm_sum) <- levels(data_subset_clean$DIFF)
  
  for (fold in 1:nfolds) {
    # Séparer train/validation pour ce fold
    val_idx <- folds[[fold]]
    train_data <- data_subset_clean[-val_idx, ]
    val_data   <- data_subset_clean[val_idx, ]
    ...
    for (true_class in levels(data_subset_clean$DIFF)) {
      for (pred_class in levels(data_subset_clean$DIFF)) {
    ...
    # Calcul F1
    f1 <- f1_score(y_true = val_data$DIFF, y_pred = pred, positive_class = levels(data_subset_clean$DIFF)[2])
...
final_model <- svm(DIFF ~ ., data = data_subset_clean, kernel = "linear", cost = best$C, probability = TRUE)
